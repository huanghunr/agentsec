...
def build_config(
    ...
    return dedent(
        ...
        workers:
          ...
          heartbeat_sec: 30
          result_contract: "home/modules/agent/orchestrator/schemas/event.schema.json"
        ...
        paths:
          ...
          contest_dir: "./contests/${{contest.slug}}"
          queue_log: "./contests/${{contest.slug}}/runtime/queue.jsonl"
          events_log: "./contests/${{contest.slug}}/runtime/events.jsonl"
          scoreboard: "./contests/${{contest.slug}}/runtime/scoreboard.json"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Create runtime orchestrator config outside repo")
    p.add_argument("--contest-slug", required=True, help="Contest identifier, e.g. tsgctf-2026")
    p.add_argument("--platform", default="ctfd", help="Platform type, e.g. ctfd/rctf/custom")
    p.add_argument("--base-url", required=True, help="Contest base URL")
    p.add_argument("--start-time", required=True, help="ISO8601 UTC, e.g. 2026-04-01T00:00:00Z")
    p.add_argument("--end-time", required=True, help="ISO8601 UTC, e.g. 2026-04-03T00:00:00Z")
    p.add_argument("--release-window", action="append", default=[], help="ISO8601 UTC, can repeat")
    p.add_argument(
        "--submission-mode",
    ...
    p.add_argument(
        "--output",
        default="",
        help="Output path. Default: ~/.local/state/agentsec/orchestrator/<contest_slug>.yaml",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    if not output:
        output = os.path.expanduser(f"~/.local/state/agentsec/orchestrator/{slug}.yaml")
